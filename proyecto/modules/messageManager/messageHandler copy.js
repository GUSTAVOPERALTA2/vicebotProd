...
if (activaReporte && requiereVerbo) {
    ...
    if (!isGroup && message.hasQuotedMsg) {
      ...
      if (incidenciaId && /^\d+$/.test(incidenciaId)) {
        const isBotMessage = /(feedback|recordatorio|tarea nueva|mensaje enviado).*id[:\s]*\d+/i;

        if (isBotMessage.test(rawQuoted)) {
          console.log('    • DM + cita → RESPUESTA detectada a mensaje del bot');
          ...
          if (isCancelWord || isCancelPhrase) {
            console.log('    • DM + cita → CANCELACIÓN detectada');
          ...
          if (isConfirmWord || isConfirmPhrase) {
            console.log('    • DM + cita → CONFIRMACIÓN detectada');
            await processConfirmation(client, message);
            return;
          }

          // 3️⃣ Feedback
          console.log('    • DM + cita → FEEDBACK detectado');
          await handleOriginResponse(client, message);
          return;
        }

        // Si no es un mensaje del bot → flujo antiguo (solicitud feedback)
